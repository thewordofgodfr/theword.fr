...
notesHelpIntro:
'העמודים הערות ולימודים מאפשרים לשמור ולארגן פסוקים ברשימות נושאיות ולהוסיף להם קטעי טקסט משלכם. תוכלו לאסוף קטעים, להוסיף מחשבות ולבנות לימודי מקרא מסודרים. ההערות והלימודים נשמרים מקומית במכשיר וזמינים ללא חשבון.',

notesHelp1Title: '1. יצירה וניהול של רשימות',

notesHelp1Body:
'העמוד הראשי מציג את כל רשימות ההערות או הלימודים. ניתן ליצור רשימה, לתת לה כותרת, לשנות את שמה או למחוק אותה. הקישו על רשימה כדי לפתוח את תוכנה. הכפתור „כל הרשימות” או „כל הלימודים” מחזיר לתצוגה הכללית. כשתפתחו שוב את העמוד, היישום יפתח אוטומטית את הרשימה האחרונה שבה השתמשתם ויעבור סמוך לפריט האחרון שלה.',

notesHelp2Title: '2. הוספת פסוקים מן המקרא',

notesHelp2Body:
'בעמוד הקריאה בחרו פסוק אחד או יותר ולאחר מכן השתמשו בכפתור הערות או לימודים. הפסוקים יתווספו לרשימות שתבחרו יחד עם המראה מקום והטקסט. ניתן לבחור כמה רשימות ולשמור את אותו פסוק במקומות שונים.',

notesHelp3Title: '3. הוספת קטעי טקסט',

notesHelp3Body:
'בנוסף לפסוקים ניתן להוסיף הערות, מחשבות, שאלות, תפילות, נקודות לדרשה או תוכן אחר. הכפתור „הוספת קטע טקסט” זמין בראש הרשימה הפתוחה ובתחתיתה. הכפתור + שליד פריט מאפשר להוסיף קטע בדיוק באותו מקום. לאחר מכן ניתן לערוך, להזיז או למחוק כל קטע.',

notesHelp4Title: '4. שימוש בפריטים וסידורם מחדש',

notesHelp4Body:
'פתחו את תפריט הפריט כדי לראות את הפעולות הזמינות. ניתן לפתוח פסוק בעמוד הקריאה, להעתיקו או לשתפו. ניתן להעתיק, לשתף או לערוך קטע טקסט. החצים מעלה ומטה משנים את סדר הפסוקים והקטעים, וניתן למחוק כל פריט בנפרד.',

notesHelp5Title: '5. העתקה או שיתוף של רשימה שלמה',

notesHelp5Body:
'בתפריט של רשימת הערות או לימוד, הכפתורים „העתקה” ו„שיתוף” מאפשרים לקבל את כל התוכן: הכותרת, מראי המקומות, טקסט הפסוקים וקטעי הטקסט האישיים. לאחר מכן ניתן להדביק אותו בהודעה או במסמך, או לשלוח אותו באמצעות יישום תואם המותקן במכשיר.',

notesHelp6Title: '6. שיתוף או העברה באמצעות קוד The Word',

notesHelp6Body:
'הכפתור „קוד” מעתיק קוד קצר המכיל את הכותרת ואת כל תוכן הרשימה. משתמש אחר ב־The Word יכול לבחור „ייבוא קוד” כדי ליצור מחדש את הרשימה במכשירו. ניתן גם להעביר תוכן בין הערות ללימודים: העתיקו את הקוד בעמוד אחד וייבאו אותו בעמוד האחר.',

notesHelp7Title: '7. ייבוא מסמך טקסט',

notesHelp7Body:
'האפשרות „ייבוא מטקסט” מאפשרת להדביק תוכן של מסמך, דוא״ל, דרשה או תוכנית לימוד. ניתן לשמור את המסמך כקטע אחד או לבקש מהיישום לחלק אותו אוטומטית למספר קטעים המופרדים בשורות ריקות. לאחר מכן תיווצר רשימה או לימוד חדשים עם הכותרת שתבחרו.',

notesHelp8Title: '8. אחסון מקומי וגיבויים',

notesHelp8Body:
'ההערות והלימודים נשמרים מקומית במכשיר ואינם מסונכרנים אוטומטית עם חשבון או שרת. אם תסירו את היישום, תאפסו את נתוניו או תמחקו את נתוני הדפדפן, התוכן עלול להימחק לצמיתות. לשמירת רשימה חשובה השתמשו בהעתקה, בשיתוף או בקוד The Word.',

notesHelp9Title: '9. רעיונות לשימוש',

notesHelp9Body:
'ניתן להשתמש בהערות לשמירת פסוקים לשינון, לכתיבת מחשבות יומיות, להכנת דרשה או לניהול רשימת תפילות. השתמשו בלימודים ליצירת לימודי מקרא מקיפים יותר על נושא או ספר, להכנת קבוצה ביתית, תוכנית הוראה או סדרת מסרים.',

notesHelp10Title: '10. שילוב הערות ולימודים',

notesHelp10Body:
'אפשר להשתמש בהערות ובלימודים יחד. לדוגמה, אספו במהירות פסוקים, מחשבות ותפילות בהערות, ולאחר מכן העבירו ללימודים את הפריטים שברצונכם להעמיק בהם באמצעות קוד The Word. שם תוכלו לסדר, להרחיב ולשתף אותם.',
  // Common
  loading: 'טוען...',
